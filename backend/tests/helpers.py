...
def extract_steps_from_code(nf_code: str, filter_valid: bool = False) -> list[str]:
    """Extract step IDs and catalog function calls from Nextflow code."""
    if not nf_code:
        return []
    ...
    if filter_valid:
        try:
            ...
            reg = get_registry()
            if not reg.valid_components and plugin:
                reg.load_plugin(plugin)
            valid_components = reg.valid_components
        except Exception:
            valid_components = set()

    # Extract Components via dynamic plugin catalog reflection
    for sym_list, path in _STEP_INCLUDE_RE.findall(nf_code):
        for sym in sym_list.split(";"):
            ...
            step_name = sym.split()[0] if sym else ""
            if not step_name:
                continue
            if filter_valid and valid_components:
                if step_name in valid_components:
