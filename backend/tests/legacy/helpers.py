...
def rate_limit_pause(seconds=15, reason="rate limit protection"):
    ...
    # Default to False (disabled for judge), then check environment
...
def run_with_retries(
    ...
    """
    all_results = []
    # Default to False, then check env
    disable_judge_rate = True
    if os.environ.get("JUDGE_RATE_LIMIT", "false").lower() == "true":
        disable_judge_rate = False
...
from core.services.tools import _inject_template, _inject_component
...
def build_test_execution_graph(store):
    ...
    from core.services.agents import hydrator_node, architect_generate_node, diagram_node, deterministic_diagram_node
    ...
    builder.add_edge("architect", "renderer")
